...
const invite_group = async (from, to, bookingId, memberId) => {
    ...
    htmlTemplate = htmlTemplate.replace('{{reject_link}}', `${process.env.CLIENT_URL}/group/reject/${bookingId}/${memberId}`);
    htmlTemplate = htmlTemplate.replace('{{accept_link}}', `${process.env.CLIENT_URL}/group/accept/${bookingId}/${memberId}`);
